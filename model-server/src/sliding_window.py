...
class SlidingWindowModel:
    ...
    def sample(self):
        ...
        # Load weights into new model that does not use the softmax output
        model = self.get_model(vocab.size, (1, *self.window_shape), False)
        model.load_weights(self.model_dir + 'sliding-window-model.h5')
        model.summary()
        ...
        logits_scaled = tf.div(model_output, temp)
        ...
        def do_sampling(width, start_string, temperature, num_generate):
                ...
                for i in range(len(start_string) - width * self.window_shape[0], num_generate):
                    ...
                    # Use model everywhere else
                        ...
                        predicted_idx = sess.run(prediction,
                                                 feed_dict={model_output: prediction_output, temp: temperature})
